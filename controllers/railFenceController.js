...
function encryptRailFence(req, res) {
  try {
    const plaintext = String(req.body.plaintext ?? req.body.text ?? '').trim();
    const rails = normalizeRails(req.body.rails);

    if (!plaintext) {
      return res.status(400).json({ error: 'Plaintext is required' });
    }

    const { matrix, pattern } = buildRailFenceMatrix(plaintext, rails);
    let ciphertext = '';

    for (let row = 0; row < rails; row += 1) {
      for (let column = 0; column < plaintext.length; column += 1) {
...
function decryptRailFence(req, res) {
  try {
    const ciphertext = String(req.body.ciphertext ?? req.body.text ?? '').trim();
    const rails = normalizeRails(req.body.rails);
    ...
    const markers = Array.from({ length: rails }, () => Array(ciphertext.length).fill(false));
    let row = 0;
    let direction = 1;

    for (let column = 0; column < ciphertext.length; column += 1) {
    ...
    for (let currentRow = 0; currentRow < rails; currentRow += 1) {
      for (let column = 0; column < ciphertext.length; column += 1) {
        if (markers[currentRow][column]) {
          matrix[currentRow][column] = ciphertext[index];
          index += 1;
        }
      }
    }
