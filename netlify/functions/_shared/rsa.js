...
function encryptMessage(message, publicKeyInput) {
  ...
  const encodedValues = encodeMessage(plaintext, publicKey.n);
...
function decryptCiphertext(ciphertext, privateKeyInput) {
  ...
  const parts = ciphertextText.split(/\s+/).filter(Boolean).map(part => toBigInt(part));
